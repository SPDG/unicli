		...
		RunE: func(cmd *cobra.Command, args []string) error {
			schema := map[string]any{
				...
				"flags": map[string]any{
					...
					"start":                "traffic report window start (RFC3339 or YYYY-MM-DD)",
					"end":                  "traffic report window end (RFC3339 or YYYY-MM-DD)",
					"timezone":             "IANA timezone for traffic reports",
					"sort":                 "traffic client ranking: download, upload, or total",
					"top":                  "max ranked traffic clients (1-100, default 10)",
					"exclude-unidentified": "omit Unidentified DPI traffic from network traffic internet",
				},
				"traffic": map[string]any{
					"max_calendar_days": network.MaxTrafficCalendarDays,
					"schemas": []string{
						network.TrafficSchemaWAN,
						network.TrafficSchemaClients,
						network.TrafficSchemaInternet,
					},
					"wan_scope":      "wan",
					"clients_scope":  "client-all-traffic",
					"internet_scope": "internet",
					"note":           "Live controller queries. POST stat/report and GET v2/traffic are read-only. Client counters are LAN-inclusive; internet is Traffic Identification (DPI).",
				},
				"commands": []map[string]any{
					...
					{"name": "network traffic wan", "legacy": true, "read_only": true, "flags": []string{"start", "end", "timezone"}},
					{"name": "network traffic clients", "legacy": true, "read_only": true, "flags": []string{"start", "end", "timezone", "sort", "top"}},
					{"name": "network traffic internet", "legacy": true, "read_only": true, "flags": []string{"start", "end", "timezone", "sort", "top", "exclude-unidentified"}},
					{"name": "network radius"},
					...
					{"name": "network sysinfo", "legacy": true},
					...
					{"name": "protect info"},
					{"name": "protect nvr"},
					{"name": "protect nvr arm", "mutation": true, "confirmation_required": true},
					{"name": "protect nvr disarm", "mutation": true, "confirmation_required": true},
					...
					{"name": "protect cameras update", "mutation": true, "confirmation_required": true},
					{"name": "protect cameras set", "mutation": true, "confirmation_required": true},
					...
					{"name": "access doors lock", "mutation": true, "confirmation_required": true},
					{"name": "access users list"},
					{"name": "access users get"},
					{"name": "access visitors list"},
					{"name": "access visitors get"},
					{"name": "access devices list"},
					{"name": "access devices get"},
					{"name": "access policies list"},
					{"name": "access policies get"},
					{"name": "access door-groups list"},
					{"name": "access user-groups list"},
