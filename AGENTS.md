...
7. For multi-gateway setups, set `UNIFI_PROFILE` or `--profile`, or rely on config `current`. Env host/key overrides the selected profile.
8. Branch on exit codes from `unicli schema` (once published), not on scraped stderr prose.
